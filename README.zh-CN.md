...
[![no proxy](https://img.shields.io/badge/no%20proxy-no%20daemon-success)](#为什么用-ccpoint)
...
取舍:ccpoint 是**会话级**的——每次 `ccpoint <name>` 在该 provider 上启动一次 Claude Code 会话。它不是"切一次就全局生效"的开关。想要持久全局切换,用 cc-switch;想要一个干净、极客、一个 `source` 就能用、不留痕迹的工具,用 ccpoint。
...
```sh
ccpoint              # 交互选择(有 fzf 用 fzf,否则数字菜单)
ccpoint glm          # 直接启动指定 provider
ccpoint list         # 列出已配置 provider
ccpoint init         # 创建配置目录
ccpoint help         # 显示帮助
```
...
每个 provider 是一个文件:`~/.config/ccpoint/providers/<name>.zsh`。`<name>` 必须是合法 shell 标识符(`[a-z_][a-z0-9_]*`),它既是 `ccpoint <name>` 的名字,也是 `_start`/`_stop` 钩子函数名的后缀。
...
**密钥永远不在 provider 文件里**——`auth_var` 指向一个环境变量,你在 `~/.zshrc` 里 export(或从密码管理器取)。AUTH_TOKEN 模式下,ccpoint 还会清掉 `ANTHROPIC_API_KEY` 防串号。

**复杂 provider**(需要起本地代理、预热等,加钩子):
```zsh
ccpoint_p=(description "本地代理" auth API_KEY auth_var PROXY_KEY)

ccpoint_start_myproxy() {        # 启动前:起进程 + 健康检查
  ccpoint_p[base_url]="http://127.0.0.1:4099"   # 可动态改写字段
  your-proxy --port 4099 &
  CCPOINT_PROXY_PID=$!           # ⚠ 不要加 local —— _stop 要读到它
  curl --retry 10 --retry-connrefused -s http://127.0.0.1:4099/health || return 1
}

ccpoint_stop_myproxy() {         # 退出:清场(ccpoint 自动 trap EXIT/INT/TERM/HUP)
  kill "$CCPOINT_PROXY_PID" 2>/dev/null
}
```

ccpoint 在调用 `_start` **之前**就装好 `EXIT` trap,所以即使 `_start` 起了进程再健康检查失败,`_stop` 也会被触发清场。推荐的 `_stop` 写法:TERM → 短暂等待 → SIGKILL 兜底(见 [`examples/opencode_go.zsh`](./examples/opencode_go.zsh))。

更多见 [`ccpoint.example.zsh`](./ccpoint.example.zsh) 和 [`examples/`](./examples/)。

## 交互选择

ccpoint 自动检测 [fzf](https://github.com/junegunn/fzf)。有 fzf:上下键 + 模糊搜索;没 fzf:数字菜单(ccpoint 刻意没用 zsh 内建的 `select`,因为它的多列布局对中文宽字符对齐有 bug)。装 fzf 体验最佳:
```sh
...
(                                          # 子 shell —— 下面所有改动都隔离在这里
  ...
  trap ccpoint_stop_<name> EXIT            # 在 _start 之前装,失败路径也能清场
  ccpoint_start_<name>                     # 健康检查等(可选)
...
- **provider 文件会被 `source`** —— 它是 zsh,能执行任意代码。ccpoint 在**启动**和**列出/渲染菜单**(为了读 `description`)时都会 source provider。所以只把可信的 provider 放进 `~/.config/ccpoint/providers/`,且顶层不要写副作用(只声明 `ccpoint_p` + 钩子)。
- ccpoint 在 source provider **之前**就解析好 `claude` 的绝对路径,所以恶意 provider 无法通过 `PATH` 或同名函数劫持 `claude`。
- provider 名做了校验(`[A-Za-z0-9._-]+`、禁 `..`),防路径穿越到 `source`。
- `kill -9` / OOM 不会触发 `EXIT` trap —— 孤儿代理可能残留。如果你的 `_start` 起了长驻进程,考虑落 PID 文件 + `ccpoint cleanup` 步骤(框架刻意没做,保持精简)。
