...
#
# Main logic, sourced by ccpoint.plugin.zsh.
# User config: ${CCPOINT_CONFIG_DIR:-${XDG_CONFIG_HOME:-~/.config}/ccpoint}/providers/*.zsh

# zsh only
if [[ -z "$ZSH_VERSION" ]]; then
  print -u 2 "ccpoint: requires zsh (current shell is not zsh)"
...
#    declare ccpoint_p + hooks at the top level (see README safety notes).
...
_ccpoint_launch() {
  ...
  if [[ ! "$name" =~ ^[A-Za-z0-9._-]+$ ]] || [[ "$name" == *..* ]]; then
    print -u 2 "ccpoint: invalid provider name '$name' (allowed: letters/digits/._- only)"
    return 1
  fi
  local f="$CCPOINT_PROVIDERS_DIR/$name.zsh"
  [[ -f "$f" ]] || { print -u 2 "ccpoint: provider '$name' not found (looked for: $f)"; return 1; }
  ...
  [[ -n "$claude_bin" ]] || { print -u 2 "ccpoint: claude not found in PATH"; return 1; }

  # Reset hook-shared state (prevent a stale CCPOINT_PROXY_PID from a previous run being read by _stop)
  ...
  (
    ...
    if [[ -z "${ccpoint_p[description]}" || -z "${ccpoint_p[auth]}" ]]; then
      print -u 2 "ccpoint: provider '$name' is missing the description/auth fields"
      exit 1
    fi
    ...
    if (( $+functions[$start_fn] )); then
      "$start_fn" || { print -u 2 "ccpoint: $name pre-launch hook failed (_start returned non-zero, proxy may not be ready)"; exit 1; }
    fi

    # Validate auth_var is set (else (P) expansion looks up an empty-named var → misleading error)
    local auth_var="${ccpoint_p[auth_var]}"
    if [[ -z "$auth_var" ]]; then
      print -u 2 "ccpoint: provider '$name' is missing the auth_var field (should name the env var holding the key)"
      exit 1
    fi
    local key_val="${(P)auth_var:-}"
    if [[ -z "$key_val" ]]; then
      print -u 2 "ccpoint: env var \$$auth_var is not set (provider=$name). Export it in ~/.zshrc and reopen the terminal."
      exit 1
    fi
    ...
    case "${ccpoint_p[auth]}" in
      ...
      *) print -u 2 "ccpoint: auth must be API_KEY or AUTH_TOKEN (got: ${ccpoint_p[auth]:-empty})"; exit 1 ;;
    esac
...
ccpoint() {
  ...
  case "$cmd" in
    "")
      # interactive select
      [[ -d "$CCPOINT_PROVIDERS_DIR" ]] || { print -u 2 "ccpoint: no providers configured. Run ccpoint init or see README."; return 1; }
      ...
      (( ${#choices[@]} == 0 )) && { print -u 2 "ccpoint: no *.zsh provider files under $CCPOINT_PROVIDERS_DIR"; return 1; }
      ...
      if (( _ccpoint_fzf )); then
        ...
        local i=1 sel
        print -u 2 "ccpoint — pick a provider:"
        for c in "${choices[@]}"; do
          print -u 2 "  $i) ${c#*|}"
          ((i++))
        done
    ...
    init)
      mkdir -p "$CCPOINT_PROVIDERS_DIR" \
        && print "ccpoint: created $CCPOINT_PROVIDERS_DIR. Drop provider files there (see ccpoint.example.zsh / examples/)."
      ;;
    help|--help|-h)
      print -r -- $'\e[1mccpoint\e[0m — point Claude Code at any provider endpoint.'
      print ""
      print "Usage:"
      print "  ccpoint              interactive select (fzf or number menu)"
      print "  ccpoint <name>       launch a specific provider"
      print "  ccpoint list         list configured providers"
      print "  ccpoint init         create the config dir $CCPOINT_PROVIDERS_DIR"
      print "  ccpoint help         show this help"
      print ""
      print "Provider config: $CCPOINT_PROVIDERS_DIR/*.zsh (see ccpoint.example.zsh / examples/)"
      ;;
    *)
      _ccpoint_launch "$cmd"
      ;;
